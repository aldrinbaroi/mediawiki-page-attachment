...
class StreamFile extends \StreamFile
{
	...
	public static function prepareForStream(
		...
		$streamForDownload = false;                                                                   // New
		if ( isset($info['downloadFileName'] )) {                                                     // New
			$streamForDownload = true;                                                                // New
			header( 'Content-type: application/x-download' );                                         // New
			header( 'Content-Disposition: attachment; filename="' . $info['downloadFileName'] . '"'); // New
		} else {                                                                                      // New
		...
		if ($streamForDownload == true) {                                                             // New
			// Do not check for client cache info                                                     // New
			// Do not send additional headers                                                         // New
		} else {                                                                                      // New
